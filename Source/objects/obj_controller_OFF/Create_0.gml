	...
	// 80 * 80 = 6400 - размер карты вертикально и горизонтально
	spc=64;
		...
		for (j=0; j<yv; j++)
		{
			...
			} else
			{
				if ( in_range(r,sandRangeMin,sandRangeMax) )
				{
					...
					instance_create_layer( (bs/2) + (i*bs), (bs/2) + (j*bs), "SAND", obj_sand_tile);
				...
				if ( in_range(r,waterRangeMin,waterRangeMax) )
				{
					...
					instance_create_layer( (bs/2) + (i*bs), (bs/2) + (j*bs), "WATER", obj_water);
				...
				if ( in_range(r,grassRangeMin,grassRangeMax) )
				{
					...
					instance_create_layer( (bs/2) + (i*bs), (bs/2) + (j*bs), "BUSH", obj_grass);
				...
				{
					//show_debug_message("Seems its a sand");
					instance_create_layer( (bs/2) + (i*bs), (bs/2) + (j*bs), "WALL", obj_metal_wall);
				...
				if ( in_range(r,brickRangeMin,brickRangeMax) )
				{
					...
					instance_create_layer( (bs/2) + (i*bs), (bs/2) + (j*bs), "BRICK", obj_brick_wall);
