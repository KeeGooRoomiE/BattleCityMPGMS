...
var bulletId = 0; // уникальный идентификатор для каждой пули
  ...
  global.squser = "tanks_usr_k9tg5ugdst5newdz";
  global.sqdatabase = "tanks_djd7d3yqggxpv99z";
  global.sqpassword = "vdMcrkckNpkPcbH6Zg";
...
sqlconnectionv.query(sqlrd,  function(err, results) {
    ...
    for(let i=0; i < tanks.length; i++){
	  ...
	  console.log(`Игрок ${tanks[i].username}  [${tanks[i].id}] добавлен из БД`);
		player = new Player({
		user_id: tanks[i].userid, // BD MINDPLAY
			...
			console.log(`Игрок ${player.username} найден на сервере ЖИВОЙ!++`);
				...
				if (players[i].isonline == 0) // Если игрок не успел вернуться за тайминг то удаляем игрока из памяти 
				{
					...
					if (typeof(players[i]) !== 'undefined')
						...
						const sql = "DELETE FROM tanks WHERE userid=? AND room=?";
...
for (let i in blocks) {
	...
	var mkrg = Number(bcount)*0.01	; // при каком колличестве регенить карту, при -25%
...
if (players.length > 0)
	{
		...
		if (!isfound)
			...
			console.log(`Игрок ${data.username} ${data.user_id} !== НЕ найден создаю...`);
							...
							const sql1 = "INSERT INTO tanks(userid, username, room, battle, amount, start_timestamp, isonline, id, x, y, imageindex, tankcolor, spriteindex, hp) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
	  ...
	  if (ispfound)
	  {
			...
			if (splayer.bb !== "6") // ББ щит
			{
				...
				if (splayer.hp<=0)
				{
					...
					const user = [ splayer.user_id, 1, Number(player.amount), Number(splayer.user_id), "killer_exit", Number(splayer.battle)];
...
////////////////////// *** POST *** //////////////////////
					...
					/// Обновить баланс
						...
						const sqldel = "DELETE FROM tanks WHERE userid=? AND room=?";
...
client.on('create_bullet', () => {
		...
		if (player.direction==0){var bullet = new Bullet(ibullet,player.x+50,player.y,15,1,player.direction,player.room);} //Right x+50
		if (player.direction==90){var bullet = new Bullet(ibullet,player.x,player.y-50,15,1,player.direction,player.room);} //Up y-50
		if (player.direction==180){var bullet = new Bullet(ibullet,player.x-50,player.y,15,1,player.direction,player.room);} //Left x-50
		if (player.direction==270){var bullet = new Bullet(ibullet,player.x,player.y+50,15,1,player.direction,player.room);} //Down y+50
